...
sqlMapConfig.xml（在resources下创建文件）
...
​		在真正使用数据时才发起查询，不用试不查询，也称之为按需加载或者懒加载
...
其中<assocition>标签的饿select属性是指定在进行teacher对象的映射时，调用的是哪个Mapper下的哪个方法。此时在TeacherDao.xml创建对应的方法
